...
- 当前活动模型快速切换与本地设置持久化
- `uTools` 上传策略切换：仅本地、仅上传模型 API、上传模型 API 与对话
- 首条用户消息驱动的会话标题自动生成
...
- 浏览器预览模式不接入 uTools 数据库，使用浏览器 `localStorage` 持久化设置与对话
- 设置始终保留一份本地副本，`uTools` 模式再按上传策略决定是否同步模型配置与对话数据
