...
3. 阿里云请求由独立 `qwenClient` 负责，基础地址默认使用 `https://dashscope.aliyuncs.com/compatible-mode/v1`，运行时自动追加 `/chat/completions`；默认模型为 `qwen3-vl-flash`，使用 Bearer API Key 和非流式多模态 Chat Completions，并显式关闭 Qwen 思考模式以优先保证图片工具响应速度。工具执行沿用当前回复的中止、轨迹和显式失败契约，不设置自动时间上限。
...
- **把图片能力写进 Provider 或回复生命周期**：会把 Qwen 协议和图片任务分支污染通用 Provider/会话流程，无法复用现有工具轨迹与中止策略。
