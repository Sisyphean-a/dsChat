...
`SettingsForm` 由 `chatAppSettings.ts` 规范化，旧持久化形状通过 `settingsDocMigration.ts` 迁移。DeepSeek 是内置默认配置；OpenAI、Kimi、MiniMax 与 custom 作为可添加配置。每份 Provider 设置都包含模型、地址、密钥、温度与能力开关。
...
- 回复启动时快照 Provider、思考开关与工具设置；后续编辑设置不能改变正在进行的回复。
